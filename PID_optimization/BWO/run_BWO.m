...
parfor i = 1:loop
	...
	tic;
    [BWO_pos(i,:),BWO_score(i),BWO_curve(i,:)]=BWO(Population,MaxIt,LB,UB,dim,fobj);
	toc;
end
toc;
disp('Done');
...
%% 打印最优参数
[minValue, minIndex] = min(BWO_score);
disp('-------------------------------------------------')
display(['最优适应度值(Best) : ',  num2str(minValue)]);
PID_best_data = BWO_pos(minIndex,:);
disp('最优参数值：')
...
name = ['BWO_',num2str(loop),'_',formattedTime, randomString, '.mat'];
save(name);
disp(name);
disp('Done');
...
plot(BWO_curve(minIndex, :), 'DisplayName', ['Curve ' num2str(i)]);
...
% 适应度平均值
figure('Position',[269   240   660   290])
plot(mean(BWO_curve),'Color','g','linewidth',1.5)
xlabel('迭代次数', 'FontSize', 12, 'FontWeight', 'bold');
...
for i = 1:size(BWO_curve, 1)  
    % 绘制第i行的曲线  
    plot(BWO_curve(i, :), 'DisplayName', ['Curve ' num2str(i)]); % 使用DisplayName属性为每条曲线指定名称    
