...
parfor i = 1:loop
	...
	tic;
    [PSO_pos(i,:),PSO_score(i),PSO_curve(i,:)]=PSO(Population,MaxIt,LB,UB,dim,fobj);
	toc;
end
toc;
disp('Done');
...
%% 打印最优参数
[minValue, minIndex] = min(PSO_score);
disp('-------------------------------------------------')
display(['最优适应度值(Best) : ',  num2str(minValue)]);
PID_best_data = PSO_pos(minIndex,:);
disp('最优参数值：')
...
name = ['PSO_',num2str(loop),'_',formattedTime, randomString, '.mat'];
save(name);
disp(name);
disp('Done');
...
plot(PSO_curve(minIndex, :), 'DisplayName', ['Curve ' num2str(i)]);
...
% 适应度平均值
figure('Position',[269   240   660   290])
plot(mean(PSO_curve),'Color','g','linewidth',1.5)
xlabel('迭代次数', 'FontSize', 12, 'FontWeight', 'bold');
...
for i = 1:size(PSO_curve, 1)  
    % 绘制第i行的曲线  
    plot(PSO_curve(i, :), 'DisplayName', ['Curve ' num2str(i)]); % 使用DisplayName属性为每条曲线指定名称    
